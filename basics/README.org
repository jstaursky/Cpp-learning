...
=*= is the _indirection_ or _dereference_ operator which enables you to access the object the pointer points to. This becomes important in situations where you want a function to modify its arguments and use call by reference (albit references provide cleaner method of accomplishing this).
e.g.,
#+begin_src C++
